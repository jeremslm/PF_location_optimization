...
Runs Bayesian and L-BFGS over a range of convergence windows
[5, 10, 15, 20, 25, 30, 35, 40, 45] to measure how solution
quality changes with window size.

Each (window, coils, lambda) result is saved to:
  examples/comparisons/closed_boundary_DIIID/
    convergence_w{window}/lambda:{REG_IN},coils:{NUM_COILS}/run_01/

JSON format is identical to opt_comp_convergence.py output.
"""
...
CONV_WINDOWS = [5, 10, 15, 20, 25, 30, 35, 40, 45]

def main(mygs, CONV_WINDOW, methods=None, **kwargs):
    ...
    RUN_FOLDER = kwargs.get('RUN_FOLDER', f'convergence_w{CONV_WINDOW}')
    ...
    print("\n" + "=" * 60)
    print(f"CONV_WINDOW={CONV_WINDOW} | Coils={NUM_COILS} | Max evals={MAX_EVALS}")
    ...
    if 'multistart_lbfgs' in methods:
        ...
        comparison.run_multistart_lbfgs(starts_window=CONV_WINDOW)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--windows', type=int, nargs='+', default=CONV_WINDOWS,
                        help='Convergence windows to sweep')
    ...
    for window in args.windows:
        for num_coils in args.coils:
            for reg_in in args.lambdas:
                print(f"\n{'='*60}")
                print(f"WINDOW={window} | NUM_COILS={num_coils} | REG_IN={reg_in}")
                print(f"{'='*60}")
                try:
                    main(
                        mygs=mygs,
